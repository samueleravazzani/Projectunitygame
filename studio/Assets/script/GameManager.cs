...
public class GameManager : MonoBehaviour
{
    ...
    public List<string> saving_Times = new List<string>();
    ...
    public void TaskDone(int category) // funzione da chiamare dopo che una task è completata
    {
        ...
        if (category == 2)
        {
            if(tasks_picked[2] == 1)
                ...
                climateEverSorted[problem_now+1] = problem_now+1; // creo una sorta di offset: //1 quiz, 2 fire, 3 fishing etc.
        }
    ...
    public void AddPublicInfoProfiles()
    {
        n_worldS_Saved.Add(0); // ho appena creato un profilo: non può aver già salvato il mondo
        ...
        saving_Times.Add(DateTime.Now.ToString("dd/MM/yyyy HH:mm"));
    ...
    public void UpdatePublicInfoProfiles()
    {
    ...
    public void DeletePublicInfoProfiles(string name_to_delete)
    {
        ...
        Debug.Log(index);
        if (index >= 0) // l'elemento esiste
        {
            // rimuovo nell'indice corrispondente
    ...
    public string[] SerializeDateTime(List<DateTime> list)
    {
